...
function getRowValue(row, possibleNames, fallback = "") {
  ...
  Object.keys(row || {}).forEach((key) => {
    lookup[normalizeName(key)] = row[key];
  });
  for (const name of possibleNames) {
    const value = lookup[normalizeName(name)];
...
function getField(fields, columnMap, displayName, fallback = "") {
  ...
  if (internalName && fields?.[internalName] !== undefined) return fields[internalName];
  if (fields?.[displayName] !== undefined) return fields[displayName];
  const wanted = normalizeName(displayName);
  const key = Object.keys(fields || {}).find((k) => normalizeName(k) === wanted);
  return key ? fields[key] : fallback;
}
...
function overlapsDateRange(startValue, endValue, rangeStart, rangeEnd) {
  ...
  const validStart = s && !isNaN(s) ? s : null;
  const validEnd = e && !isNaN(e) ? e : null;
...
export default function App() {
  ...
  async function connect() {
    try {
      ...
      console.error(error);
      if (error.message?.includes("Login is still processing") || error.message?.includes("interaction_in_progress")) {
        setStatus("Microsoft login is processing. Click Connect to SharePoint again.");
        return;
      }
  ...
  async function loadFromSharePoint(connection = sp) {
    ...
    const summaryItems = await getListItems(connection.siteId, connection.summaryListId);
    const receiptItems = await getListItems(connection.siteId, connection.receiptsListId);
    const cashSaleItems = await getListItems(connection.siteId, connection.cashSalesListId);
    const fundraiserItems = await getListItems(connection.siteId, connection.specialFundraisersListId);
    const transferItems = await getListItems(connection.siteId, connection.transfersListId);
...
grossSales: moneyToNumber(
  getField(f, connection.summaryColumnMap, "Gross Sales", 0)
        ...
        netProfit: moneyToNumber(getField(f, connection.summaryColumnMap, "Net Profit", 0)),
        needsReceipts: Boolean(getField(f, connection.summaryColumnMap, "Needs Receipt", false)),
        inactive: Boolean(getField(f, connection.summaryColumnMap, "Inactive", false)),
        receipts: [],
      };
    });
    ...
    const loadedCashSales = cashSaleItems.map((item) => {
      ...
      return {
        id: crypto.randomUUID(),
        raffleNumber: cleanRaffleNumber(getField(f, connection.cashSalesColumnMap, "Raffle Number", "")),
        cashDate: getField(f, connection.cashSalesColumnMap, "Cash Date", ""),
        amount: moneyToNumber(getField(f, connection.cashSalesColumnMap, "Amount", 0)),
        notes: getField(f, connection.cashSalesColumnMap, "Notes", ""),
        inactive: Boolean(getField(f, connection.cashSalesColumnMap, "Inactive", false)),
      };
    });
    ...
    const loadedSpecialFundraisers = fundraiserItems.map((item) => {
      ...
      return {
        id: crypto.randomUUID(),
        title: getField(f, connection.specialFundraisersColumnMap, "Title", ""),
        fundraiserDate: getField(f, connection.specialFundraisersColumnMap, "Fundraiser Date", ""),
        amountRaised: moneyToNumber(getField(f, connection.specialFundraisersColumnMap, "Amount Raised", 0)),
        expenses: moneyToNumber(getField(f, connection.specialFundraisersColumnMap, "Expenses", 0)),
        notes: getField(f, connection.specialFundraisersColumnMap, "Notes", ""),
        inactive: Boolean(getField(f, connection.specialFundraisersColumnMap, "Inactive", false)),
      };
    });
    ...
    const loadedTransfers = transferItems.map((item) => {
      ...
      return {
        ...
        transferDate: getField(f, connection.transfersColumnMap, "Transfer Date", ""),
        amount: moneyToNumber(getField(f, connection.transfersColumnMap, "Amount", 0)),
        purpose: getField(f, connection.transfersColumnMap, "Purpose", ""),
        inactive: Boolean(getField(f, connection.transfersColumnMap, "Inactive", false)),
      };
    });
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        setRows((existingRows) => {
          ...
          const newRows = incomingRows.filter((row) => !existingKeys.has(makeRowKey(row)));
          return [...existingRows, ...newRows];
        });
        setUploadedFiles((prev) => [...prev, { name: file.name, uploadedAt: new Date().toLocaleString(), rowCount: incomingRows.length }]);
      },
    });
    event.target.value = "";
  }
  ...
  function toggleInactive(raffleNumber) {
    setRaffleData((prev) => {
      const current = prev[raffleNumber] || blankRaffleData();
      return { ...prev, [raffleNumber]: { ...current, inactive: !Boolean(current.inactive) } };
    });
  }
  ...
  function addReceipt(raffleNumber) {
    setRaffleData((prev) => {
      ...
      return {
        ...
        [raffleNumber]: {
          ...current,
          receipts: [...(current.receipts || []), { id: crypto.randomUUID(), vendor: "", description: "", amount: "" }],
        },
      };
    });
  }
  ...
  function updateReceipt(raffleNumber, receiptId, field, value) {
    setRaffleData((prev) => {
      ...
      return {
        ...
        [raffleNumber]: {
          ...current,
          receipts: (current.receipts || []).map((r) => (r.id === receiptId ? { ...r, [field]: value } : r)),
        },
      };
    });
  }
  ...
  function deleteReceipt(raffleNumber, receiptId) {
    setRaffleData((prev) => {
      const current = prev[raffleNumber] || blankRaffleData();
      return { ...prev, [raffleNumber]: { ...current, receipts: (current.receipts || []).filter((r) => r.id !== receiptId) } };
    });
  }
  ...
  const cashSalesByRaffle = useMemo(() => {
    const grouped = {};
    cashSales.filter((sale) => !sale.inactive).forEach((sale) => {
      const raffleNumber = cleanRaffleNumber(sale.raffleNumber);
      if (!raffleNumber) return;
      grouped[raffleNumber] = (grouped[raffleNumber] || 0) + moneyToNumber(sale.amount);
    });
    return grouped;
  }, [cashSales]);
  ...
  const csvReport = useMemo(() => {
    ...
    rows.forEach((row) => {
      ...
      const grossSales = moneyToNumber(getRowValue(row, ["Gross Sales", "Gross sales", "Gross Sale", "Gross Amount", "Item Gross Sales", "Total Sales", "Net Sales", "Amount"], 0));
      const feesFromSquare = moneyToNumber(getRowValue(row, ["Fees", "Processing Fees", "Square Fees", "Card Processing Fees", "Fee"], 0));
      const dateValue = getRowValue(row, ["Date", "Transaction Date"], "");
      const transactionId = getRowValue(row, ["Transaction ID", "Transaction Id"], "");
      if (!grouped[raffleNumber]) grouped[raffleNumber] = { raffleNumber, prize, onlineSold: 0, grossSales: 0, squareFeesActual: 0, transactions: new Set(), dates: [] };
      ...
      if (transactionId) grouped[raffleNumber].transactions.add(transactionId);
      if (dateValue) grouped[raffleNumber].dates.push(dateValue);
    });

    return Object.values(grouped).map((raffle) => {
      const validDates = raffle.dates.map((d) => new Date(d)).filter((d) => !isNaN(d)).sort((a, b) => a - b);
      const ranFrom = validDates.length ? formatDate(validDates[0]) : "";
      const ranUntil = validDates.length ? formatDate(validDates[validDates.length - 1]) : "";
      const monthsRan = [...new Set(validDates.map((d) => formatMonthYear(d)))].filter(Boolean).join(", ");
      const transactionCount = raffle.transactions.size;
      const squareFees = raffle.squareFeesActual !== 0 ? Math.abs(raffle.squareFeesActual) : raffle.grossSales * 0.029 + transactionCount * 0.3;
      const existingSaved = savedSummary.find((saved) => saved.raffleNumber === raffle.raffleNumber);
      return {
  ...
  const displayReport = useMemo(() => {
    ...
    csvReport.forEach((r) => {
      ...
      merged[r.raffleNumber] = { ...existing, ...r, prize: r.prize || existing.prize || "", stock: existing.stock ?? r.stock ?? 0, inactive: existing.inactive ?? r.inactive ?? false, receipts: existing.receipts || r.receipts || [] };
    });
 ...
 const printReport = useMemo(
  ...
  const raffleTotals = useMemo(() => {
    return displayReport.reduce((sum, r) => {
  ...
  const specialFundraiserTotals = useMemo(() => {
    return specialFundraisers.filter((r) => !r.inactive).reduce((sum, r) => {
      const raised = moneyToNumber(r.amountRaised);
      const expenses = moneyToNumber(r.expenses);
      sum.amountRaised += raised;
      sum.expenses += expenses;
      sum.net += raised - expenses;
      return sum;
    }, { amountRaised: 0, expenses: 0, net: 0 });
  }, [specialFundraisers]);

  const transferTotals = useMemo(() => {
    return treasurerTransfers.filter((r) => !r.inactive).reduce((sum, r) => {
      sum.amount += moneyToNumber(r.amount);
      return sum;
    }, { amount: 0 });
  }, [treasurerTransfers]);

  const finalNetRemaining = raffleTotals.netProfit + specialFundraiserTotals.net - transferTotals.amount;
  ...
  async function saveToSharePoint() {
    ...
    const reportSnapshot = displayReport.map((r) => {
      ...
      const receipts = Array.isArray(stateReceipts) && stateReceipts.length > 0 ? stateReceipts : Array.isArray(reportReceipts) ? reportReceipts : [];
      const receiptCost = calculateReceiptCost(receipts);
      const grossSales = Number(r.grossSales || 0);
      const squareFees = Number(r.squareFees || 0);
      const totalExpenses = squareFees + receiptCost;
      const netProfit = grossSales - totalExpenses;
      return { ...r, receipts, receiptCost, totalExpenses, netProfit, needsReceipts: receiptCost <= 0 };
    });

    const totalReceiptRows = reportSnapshot.reduce((count, raffle) => count + (raffle.receipts?.length || 0), 0);
    const hadReceiptsBefore = Object.values(raffleData).some((raffle) => Array.isArray(raffle.receipts) && raffle.receipts.length > 0);

    if (hadReceiptsBefore && totalReceiptRows === 0) {
      alert("Save stopped because no receipt rows were found in the app state. This would wipe out the SharePoint receipt list. Reconnect / Reload SharePoint and try again.");
      setStatus("Save stopped to protect existing receipts.");
      return;
    }

    if (!window.confirm(`This will replace SharePoint summary/manual lists and rewrite ${totalReceiptRows} receipt row(s). Continue?`)) return;
    ...
    try {
      ...
      if (totalReceiptRows > 0) {
        ...
        for (const r of reportSnapshot) {
          for (const receipt of r.receipts || []) {
            ...
            if (!receipt.vendor && !receipt.description && amount === 0) continue;
            await createListItem(sp.siteId, sp.receiptsListId, sp.receiptsColumnMap, {
              Title: `Raffle #${r.raffleNumber}`,
              "Raffle Number": String(r.raffleNumber),
              Prize: r.prize || "",
      ...
      for (const sale of cashSales) {
        ...
        if (!sale.raffleNumber && amount === 0 && !sale.notes) continue;
        await createListItem(sp.siteId, sp.cashSalesListId, sp.cashSalesColumnMap, {
      ...
      for (const fundraiser of specialFundraisers) {
        ...
        if (!fundraiser.title && amountRaised === 0 && expenses === 0) continue;
        await createListItem(sp.siteId, sp.specialFundraisersListId, sp.specialFundraisersColumnMap, {
      ...
      for (const transfer of treasurerTransfers) {
        ...
        if (!transfer.title && amount === 0 && !transfer.purpose) continue;
        await createListItem(sp.siteId, sp.transfersListId, sp.transfersColumnMap, {
    ...
    <div className="app">
      ...
      <section className="upload-card no-print">
        <div className="button-row">
          <button onClick={connect}>{sp ? "Reconnect / Reload SharePoint" : "Connect to SharePoint"}</button>
          <label className="upload-box"><input type="file" accept=".csv" onChange={handleFileUpload} />Upload Square CSV</label>
        ...
        {uploadedFiles.length > 0 && (
          <div className="uploaded-files">
            ...
            {uploadedFiles.map((file, index) => <p key={`${file.name}-${index}`}>{file.name} — {file.uploadedAt} — {file.rowCount} rows</p>)}
          </div>
        )}
      </section>

      {displayReport.length === 0 ? (
        <section className="empty"><h2>No raffle data loaded yet</h2><p>Click <strong>Connect to SharePoint</strong> to load saved data, or upload a Square CSV to add new sales data.</p></section>
      ) : (
        <>
          ...
          <section className="report-card no-print">
            ...
            {displayReport.map((r) => {
              ...
              return <div className="receipt-box" key={`receipts-${r.raffleNumber}`}><div className="receipt-header"><div><h3>Raffle #{r.raffleNumber} <span className={r.inactive ? "status-pill inactive" : "status-pill"}>{r.inactive ? "Inactive" : "Active"}</span></h3><p>{r.prize}</p></div><button onClick={() => addReceipt(r.raffleNumber)}>Add Receipt</button></div>{receipts.length === 0 ? <p className="receipt-warning">No receipts entered yet.</p> : <div className="receipt-list">{receipts.map((receipt) => <div className="receipt-row" key={receipt.id}><input type="text" placeholder="Vendor / Store" value={receipt.vendor} onChange={(e) => updateReceipt(r.raffleNumber, receipt.id, "vendor", e.target.value)} /><input type="text" placeholder="Description" value={receipt.description} onChange={(e) => updateReceipt(r.raffleNumber, receipt.id, "description", e.target.value)} /><input type="number" placeholder="Amount" value={receipt.amount} onChange={(e) => updateReceipt(r.raffleNumber, receipt.id, "amount", e.target.value)} /><button className="danger" onClick={() => deleteReceipt(r.raffleNumber, receipt.id)}>Delete</button></div>)}</div>}</div>;
            })}
          </section>
          ...
          <section className="report-card no-print"><div className="receipt-header"><div><h2>Treasurer Transfers</h2><p>Money moved out of fundraising net for a stated purpose.</p></div><button onClick={addTreasurerTransfer}>Add Transfer</button></div><div className="receipt-list">{treasurerTransfers.map((t) => <div className="manual-row" key={t.id}><input type="text" placeholder="Title" value={t.title} onChange={(e) => updateTreasurerTransfer(t.id, "title", e.target.value)} /><input type="date" value={t.transferDate} onChange={(e) => updateTreasurerTransfer(t.id, "transferDate", e.target.value)} /><input type="number" placeholder="Amount" value={t.amount} onChange={(e) => updateTreasurerTransfer(t.id, "amount", e.target.value)} /><input type="text" placeholder="Purpose" value={t.purpose} onChange={(e) => updateTreasurerTransfer(t.id, "purpose", e.target.value)} /><button className="danger" onClick={() => deleteTreasurerTransfer(t.id)}>Delete</button></div>)}</div></section>
        </>
      )}
    </div>
  );
